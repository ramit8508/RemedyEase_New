...
import { fetchWithRetry, wakeUpBackends } from "../../utils/backendUtils.js";
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const fullURL = `${API_CONFIG.ENDPOINTS.USERS}/register`;
      console.log("Backend API URL:", fullURL);

      // Direct request to backend server
      ...
      if (!res.ok) {
        ...
        try {
          ...
          errorText = errorData.message || errorData.error || `Server Error ${res.status}`;
        } catch (e) {
          errorText = `Server returned ${res.status} - ${res.statusText}`;
        }
      ...
      if (err.name === "AbortError") {
        setMessage("⏱️ Request timed out. Please try again.");
