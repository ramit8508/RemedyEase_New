...
const createTransporter = () => {
  ...
  if (!user || !pass) {
    console.warn("[EmailService] Warning: EMAIL_USER or EMAIL_PASSWORD environment variables are not configured.");
  }
...
export const sendSignupOtpEmail = async (email, otp, name = "") => {
  ...
  const mailOptions = {
    from: `"RemedyEase" <${process.env.EMAIL_USER || "noreply@remedyease.com"}>`,
    to: email,
...
export const sendLoginOtpEmail = async (email, otp) => {
  ...
  const mailOptions = {
    from: `"RemedyEase Security" <${process.env.EMAIL_USER || "noreply@remedyease.com"}>`,
    to: email,
